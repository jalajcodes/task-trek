...
const EntriesDisplay = () => {
  const [
    { entries, filter, search, tag, sortBy, isLoading, status },
    dispatch,
  ] = useEntryContext();
  ...
  let entriesArray = getEntriesArray(entries, search, tag, status);
  ...
  const infoText = showInfoText(filter, search, tag, currentFilter, status);
    ...
    <div>
      <div className={classes.flexedBar}>
        <div className={classes.textAndButton}>
          {(tag || filter || search || status) && backButton()}
          <Typography
            variant="h6"
